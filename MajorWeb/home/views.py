...
def redu(cate, countr):
    data = pd.read_csv('C:\\Users\\shash\\Desktop\\Data_Science\\Major2020\\webScrapping/final_dataset.csv', encoding= 'unicode_escape')
    if(countr!='empty'):
...
def cosineSimilairty(cate, countr, productDescription, hashtags):
    ...
    df=[]
    df.insert(0,{'name':'aaa', 'description':productDescription,'tagged_names':hashtags})
...
def normalize(data):
    ...
    data = pd.DataFrame(min_max_scaler.fit_transform(x), index=data.index, columns=data.columns)
    return data

def allto(data):
    data['cumulative']=((1/3)*data['cosine_sim'])+((1/3)*data['acc_to_post'])+((1/3)*data['acc_to_videos'])
    return data
...
def predict(request):
    if request.method=='GET':
        ...
        print(data)
        cos_data=pd.Series
        cos_data=cosineSimilairty(category, country, productDescription, hashtags)
        ...
        link_data = pd.read_excel('C:\\Users\\shash\\Desktop\\Data_Science\\Major2020\\webScrapping\\Influencer_with_links.xlsx')
        ...
        data['acc_to_post']=0.65*data['average_post_comments'] + 0.35*data['average_post_likes']
        ...
        data['acc_to_videos']=0.65*data['average_videos_comments'] + 0.35*data['average_videos_views']
        ...
        # print("posts")
        # print(onBasisPostLinks[0:6])
        # print("videos")
        # print(onBasisVideosLinks[0:6])
        # print("cosine")
        # print(onBasisCosineLinks[0:6])
        # print("all")
        # print(onBasisAllLinks[0:6])
