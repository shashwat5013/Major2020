...
try:
    for i in range(1,number_of_post+1):
        ...
        if first==0:
            ...
            move=driver.find_element_by_xpath('/html/body/div[4]/div[1]/div/div/a[2]')
        time.sleep(5)
        try:
            text=driver.find_element_by_xpath('/html/body/div[4]/div[2]/div/article/div[3]/div[1]/ul/div/li/div/div/div[2]')
            about_post=str(text.text)
            print(about_post)
        except:
            print("not known")
        try:
            count=driver.find_element_by_xpath('/html/body/div[4]/div[2]/div/article/div[3]/section[2]/div')
            likes=(count.text)
        except:
            likes=0
        try:
            date=driver.find_element_by_xpath('/html/body/div[4]/div[2]/div/article/div[3]/div[2]/a')
            date=(date.text)
        except:
            date="not known"
    ...
    df.to_csv('C:\\Users\\shash\\Desktop\\Data_Science\\Major2020\\webScrapping\\about_post\\ranveersingh.csv',index=False)        
    driver.close()